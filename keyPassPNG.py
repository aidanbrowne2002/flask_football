...
def generate_player_plot(player_id, type, postgreSQL_pool):
    if type == 1:
        assist = 1
        keypass = 0
    elif type == 0:
    ...
    query = """SELECT
        ...
        GROUP BY
            ...
            e.y;
        """
    ps_cursor.execute(query, (player_id, assist, keypass))
    ...
    start_x = np.array([float(i[1]) for i in result])
    start_y = np.array([float(i[2]) for i in result])
    end_x = np.array([float(i[4]) for i in result])
    end_y = np.array([float(i[5]) for i in result])
    ...
    try:
        ...
        # Show the plot
        if (type == 1):
            fig1.savefig(f'static/images/assists/{player_id}.png', transparent=True)
        if (type == 0):
            fig1.savefig(f'static/images/keypasses/{player_id}.png', transparent=True)
